...
| `subId`        | ID подписки: при совпадении сервер (основной + включённые юзеры) попадает в вывод подписки во всех форматах — ссылка `naive+https://` в обычном, прокси `http` с TLS в Clash, outbound `http` поверх TLS в JSON; пусто = не включать |
| `ipLimit`      | макс. число IP **на пользователя**; лишние баннятся через nftables (`inet 3xui_naive_ban`, с таймаутом → авто-разбан). Включает per-site JSON access-log. **Linux + root**; 0 = без лимита |
...
| `total`        | лимит трафика в байтах (0 = безлимит); при достижении процесс останавливается |
| `expiryTime`   | абсолютная дата истечения в мс (0 = бессрочно); после неё процесс останавливается |
| `trafficReset` | расписание авто-сброса счётчиков: `never` / `day` / `week` / `month` |
| `users`        | доп. пользователи `[{username, password, enable}]` — каждый включённый становится отдельной строкой `basic_auth` и получает свой URL/QR |

При достижении лимита или истечении срока naive-traffic-job (раз в 30с)
останавливает процесс, не трогая `enable` — после сброса счётчика или
продления срока сервер поднимается автоматически.
