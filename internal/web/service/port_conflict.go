...
func checkPortConflictTx(db *gorm.DB, inbound *model.Inbound, ignoreId int) (*portConflictDetail, error) {
	...
	// NaiveProxy servers are panel-managed local processes, not inbound rows.
	naive, err := naivePortConflictsInbound(db, inbound)
	if err != nil {
		return nil, err
	}
	if naive != nil {
		return &portConflictDetail{
			Tag:        "naive",
			Remark:     strings.TrimSpace("NaiveProxy " + naive.Remark),
			Listen:     naive.Listen,
			Port:       naive.Port,
			Transports: transportTCP,
		}, nil
