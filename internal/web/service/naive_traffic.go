...
func rebuildNftCounters(servers []*model.NaiveServer) error {
	...
	cmd := exec.CommandContext(context.Background(), nftBin, "-f", "-")
...
func sampleNftCounters() (map[int]naiveTrafficDelta, error) {
	...
	cmd := exec.CommandContext(context.Background(), nftBin, "-j", "reset", "counters", "table", "inet", nftTable)
...
func teardownNftCounters() error {
	...
	cmd := exec.CommandContext(context.Background(), nftBin, "delete", "table", "inet", nftTable)
	if out, err := cmd.CombinedOutput(); err != nil {
