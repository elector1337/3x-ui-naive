...
func naivePortInUse(db *gorm.DB, port int, listen string, ignoreId int) (*model.NaiveServer, error) {
...
func crossCheckNaivePort(srv *model.NaiveServer, ignoreNaiveId int) error {
	...
	if other, err := naivePortInUse(database.GetDB(), srv.Port, srv.Listen, ignoreNaiveId); err != nil {
		return err
	} else if other != nil {
		label := other.Remark
		if label == "" {
...
func naivePortConflictsInbound(db *gorm.DB, inbound *model.Inbound) (*model.NaiveServer, error) {
	...
	return naivePortInUse(db, inbound.Port, inbound.Listen, 0)
}
