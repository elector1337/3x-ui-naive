...
func (s *Server) initRouter() (*gin.Engine, error) {
	...
	s.api = controller.NewAPIController(g, s.naiveService)
...
func (s *Server) start(restartXray bool, startTgBot bool) (err error) {
	...
	_, _ = s.cron.AddJob("@every 30s", job.NewNaiveTrafficJob(s.naiveService))
	...
	_, _ = s.cron.AddJob("@every 20s", job.NewNaiveIpJob(s.naiveService))

	// periodic naive counter resets (mirrors the inbound reset cadence)
	_, _ = s.cron.AddJob("@daily", job.NewNaiveTrafficResetJob(s.naiveService, "day"))
	_, _ = s.cron.AddJob("@weekly", job.NewNaiveTrafficResetJob(s.naiveService, "week"))
	_, _ = s.cron.AddJob("@monthly", job.NewNaiveTrafficResetJob(s.naiveService, "month"))
