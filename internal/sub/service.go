...
func (s *SubService) getSubs(subId string) ([]string, []string, int64, xray.ClientTraffic, error) {
	...
	// NaiveProxy servers tagged with this subId (primary + enabled users).
	// Naive runs as a separate process with no per-client traffic stats, so
	// these only contribute links, not usage figures.
	var naiveLinks []string
	if s.naiveService != nil {
		naiveLinks = s.naiveService.SubLinks(subId)
	}

	if len(inbounds) == 0 && len(externalLinks) == 0 && len(naiveLinks) == 0 {
